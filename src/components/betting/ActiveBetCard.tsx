...
import {
  generateCompleteRaceData,
  submitRaceResult,
} from "@/utils/raceGenerator";
...
export const ActiveBetCard: React.FC<ActiveBetCardProps> = ({
  refreshTrigger,
}) => {
    ...
    try {
      ...
      if (result.autoCompleted) {
        if (result.outcome === "completed") {
          ...
          const winnerPayout =
            typeof result.winnerPayout === "string"
  ...
  const handleCancelBet = async (betId: string) => {
    if (
      !confirm(
        "Are you sure you want to cancel this bet? Your wager will be refunded.",
      )
    ) {
      return;
    }
      ...
      {bets.length === 0 ? (
        ...
        <div className="space-y-4">
          {bets.map((bet) => {
            ...
            const userSubmitted = isCreator
              ? bet.creatorRaceValidationId
              : bet.acceptorRaceValidationId;
            const opponentSubmitted = isCreator
              ? bet.acceptorRaceValidationId
              : bet.creatorRaceValidationId;
            const showRandomRaceButton =
              bet.status === "active" && !userSubmitted;
            ...
            const winnerPayout = bet.winnerPayout
              ? typeof bet.winnerPayout === "string"
                ? parseFloat(bet.winnerPayout)
                : bet.winnerPayout
              : null;
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <p className="text-sm text-purple-300">
                      {track?.name || bet.trackId} • {betAmount.toFixed(2)}{" "}
                      gains
                    </p>
                  </div>
                  <div className="text-right">
                    ...
                    {bet.status === "open" && (
                      <p className="text-xs text-purple-400">
                        Expires {dayjs(bet.expiresAt).fromNow()}
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  <div className="bg-purple-900/30 rounded-lg p-3 mb-3">
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-sm">
                        ...
                        <span
                          className={
                            userSubmitted ? "text-green-400" : "text-yellow-400"
                          }
                        >
                ...
                {bet.status === "completed" && bet.winnerId && (
                  <div
                    className={`rounded-lg p-3 mb-3 ${isWinner ? "bg-green-900/30" : "bg-red-900/30"}`}
                  >
